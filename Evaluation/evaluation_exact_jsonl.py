...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Evaluate LLaVA jsonl outputs per-file + global: accuracy + F1 (macro/micro/weighted)."
    )
    p.add_argument("--in_dir", type=str, default="output_llava_recog_asso/lora-infer", help="Root dir containing jsonl files")
    p.add_argument("--out_dir", type=str, default="result_llava_recog_asso/lora-infer", help="Root dir to save json results with same structure")
    p.add_argument("--ext", type=str, default=".jsonl", help="Input extension to search (default: .jsonl)")
    p.add_argument("--casefold", action=argparse.BooleanOptionalAction, default=True, help="Lowercase normalization")
    p.add_argument("--strip_punct", action=argparse.BooleanOptionalAction, default=False, help="Strip punctuation before compare")
    p.add_argument("--print_topk", type=int, default=15, help="Top-k groups to print per breakdown")
    p.add_argument("--print_per_file", action=argparse.BooleanOptionalAction, default=True, help="Print per-file report")
    p.add_argument("--include_confusion", action=argparse.BooleanOptionalAction, default=False, help="Save confusion counts per file (can be big)")
    return p.parse_args()
...
def compute_f1_from_confusion(conf: ConfusionCounter) -> Dict[str, Any]:
    ...
    # include predicted labels too (so FP-only classes are counted in macro if desired)
    ...
    # micro precision/recall useful too
    return {
        "macro_f1": macro_f1,
        "micro_f1": micro_f1,
...
def print_topk(title: str, rows: List[Dict[str, Any]], topk: int, key_fields: Optional[List[str]] = None) -> None:
    ...
    for r in rows[:topk]:
        ...
        f1_val = r.get("macro_f1", r.get("f1", None))
        f1_str = f"  macroF1= {f1_val:.4f}" if f1_val is not None else ""
        print(f"    {key_str:60s}  n={r['n']:6d}  acc= {r['acc_percent']}{f1_str}")
...
def main() -> None:
    ...
    jsonl_paths = sorted(in_root.rglob(f"*{args.ext}"))
    if not jsonl_paths:
    ...
    # Also compute F1 per group (global) if you want later; for now keep confusion per group
    ...
    for jp in jsonl_paths:
        ...
        with jp.open("r", encoding="utf-8") as f:
            for line_idx, line in enumerate(f, start=1):
                ...
                gt_raw = safe_get(obj, "gt")

                main_tag = safe_get(obj, "main_tag")
                sub_tag = safe_get(obj, "sub_tag")
                sub_sub_tag = safe_get(obj, "sub_sub_tag")
                ...
                # update file acc + file confusion
                file_acc.add(ok)
                ...
                records_out.append({
                    ...
                    "ori_img_tag": obj.get("ori_img_tag", None),
                })

        # ---- Per-file metrics ----
        file_f1 = compute_f1_from_confusion(file_conf)

        # For per-file tag breakdown: compute F1 for each group too (can be many, but ok)
        ...
        for r in file_by_subsub_rows:
            k = r["key"]
            r["f1"] = compute_f1_from_confusion(file_conf_by_subsub[k])["macro_f1"]*100

        file_by_combo_rows = counter_dict_to_list(
            file_by_combo, key_names=["main_tag", "sub_tag", "sub_sub_tag"]
        )
        for r in file_by_combo_rows:
            key = (r["main_tag"], r["sub_tag"], r["sub_sub_tag"])
            r["macro_f1"] = compute_f1_from_confusion(file_conf_by_combo[key])["macro_f1"]
            r["micro_f1"] = compute_f1_from_confusion(file_conf_by_combo[key])["micro_f1"]
            r["weighted_f1"] = compute_f1_from_confusion(file_conf_by_combo[key])["weighted_f1"]
        ...
        if args.include_confusion:
            # Warning: potentially huge if many classes
            payload["confusion_pairs"] = [
                {"gt": g, "pred": p, "count": c}
                for (g, p), c in file_conf.pairs.most_common()
            ]
        ...
        if args.print_per_file:
            ...
            print(f"  ✅ file_F1: macro={file_summary['macro_f1']:.6f} | micro={file_summary['micro_f1']:.6f} | weighted={file_summary['weighted_f1']:.6f}")
            print(f"  - samples: {file_summary['num_samples']}  correct: {file_summary['correct']}  classes(seen): {file_summary['num_classes']}")

            # Print topk by accuracy (already sorted)
            ...
            for r in file_summary["by_combo_main_sub_subsub"][: args.print_topk]:
                ...
                print(f"    {key_str:60s}  n={r['n']:6d}  acc= {r['acc_percent']}  macroF1= {r['macro_f1']*100:.4f}")
    ...
    print(f"  - Files scanned   : {len(jsonl_paths)}")
    ...
    print(f"  - F1              : macro={global_f1['macro_f1']:.6f} | micro={global_f1['micro_f1']:.6f} | weighted={global_f1['weighted_f1']:.6f}")
    print(f"  - Classes(seen)   : {global_f1['num_classes']}")
    print("=" * 90)

    # Global breakdown lists (add macro_f1 for each tag group)
